...
This project includes sample credentials for external services in the `IMPORT FILE/` folder. These are provided for testing and development purposes:

- **Elasticsearch**: Cloud instance credentials in `IMPORT FILE/credentials.txt`
- **Broadcasting**: Laravel Reverb configuration in `IMPORT FILE/credentials.txt`
- **Mailing**: Mailtrap SMTP credentials in `IMPORT FILE/credentials.txt`
- **Sample Data**: Product CSV file `IMPORT FILE/products_1000.csv` for bulk import testing

**Note**: For production use, replace these with your own service credentials.
...
To enable Elasticsearch integration, ensure the following environment variables are set in your `.env` file (sample credentials provided in `IMPORT FILE/credentials.txt`):

```env
ELASTIC_HOST=https://14a4ab6a1e3a46b9a68b3b9f9d4a7875.us-east1.gcp.elastic-cloud.com:443
ELASTIC_USERNAME=elastic
ELASTIC_PASSWORD=TZOlml8ZgI4g6DomSnQ43b68
ELASTIC_INDEX=products-index
```

**Note**: The provided credentials are for a cloud Elasticsearch instance. For local development, you can set up your own Elasticsearch server.
...
### 6. Database Setup
For local development, the project uses MySQL by default. Create a MySQL database and update the connection details in `.env`.
...
Copy `.env.example` to `.env` and configure the following key variables. Sample credentials are provided in `IMPORT FILE/credentials.txt`:
...
REVERB_APP_ID=app_8f3b1c2d
REVERB_APP_KEY=key_9a7d6e5f
REVERB_APP_SECRET=secret_4b2c1a0d
REVERB_HOST=api.reverb.example.com
REVERB_PORT=443
REVERB_SCHEME=https
...
MAIL_PORT=2525
MAIL_USERNAME=78856498c9a058
MAIL_PASSWORD=1f9d86665f0987
MAIL_ENCRYPTION=tls
...
A sample product CSV file is provided in the `IMPORT FILE/` folder: `IMPORT FILE/products_1000.csv`
...
3. **Configure environment variables** in `.env` (sample credentials provided in `IMPORT FILE/credentials.txt`):
    ```env
    BROADCAST_CONNECTION=reverb
    REVERB_APP_ID=app_8f3b1c2d
    REVERB_APP_KEY=key_9a7d6e5f
